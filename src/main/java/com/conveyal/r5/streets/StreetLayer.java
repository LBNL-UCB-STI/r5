...
public class StreetLayer implements Serializable, Cloneable {
    ...
    private void makeEdge(Way way, int beginIdx, int endIdx, Long osmID) {
        ...
        // Catch self-loops from ANY source
        ...
        try {
            ...
            edgeLengthMillimeters = getEdgeLengthMillimeters(nodes);
        }
    ...
    public int getOrCreateVertexNear(double lat, double lon, StreetMode streetMode, double linkRadiusMeters) {
        ...
        // Split.findOnEdge() now correctly calculates distances using spherical distance for both
        // distance0_mm and distance1_mm, so we can rely on these values.
